package com.ethan.customControls.service;
...
public class MusicService extends Service {
    ...
    private void sendNotify(Context ctx, String song, boolean isPlaying, int progress) {
        ...
        // 创建一个跳转到活动页面的意图
        Intent intent = new Intent(ctx, CustomControlMainActivity.class);
        // 创建一个用于页面跳转的延迟意图
        PendingIntent clickIntent = PendingIntent.getActivity(ctx,
                R.string.book_name, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        // 创建一个通知消息的建造器
        Notification.Builder builder = new Notification.Builder(ctx);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            // Android 8.0开始必须给每个通知分配对应的渠道
            builder = new Notification.Builder(ctx, getString(R.string.book_name));
        }
        builder.setContentIntent(clickIntent) // 设置内容的点击意图
                .setSmallIcon(R.mipmap.tt_s) // 设置应用名称左边的小图标
                // 设置通知栏右边的大图标
                .setLargeIcon(BitmapFactory.decodeResource(getResources(), R.mipmap.tt))
